...
InfraDrishti has a defined acquisition architecture (discover → download → validate → preprocess → analyse). In the current synchronous API, this is governed by a **validated data cache**:
...
flowchart TD
    ...
    B --> C[Verify endpoints within validated cache]
    C --> |in coverage| D[Reuse validated cache]
    C --> |out of coverage| E[Return DATA_COVERAGE_BLOCKER]
    D --> F[Run analysis]
    F --> G[Write provenance]
    G --> H[Delete temporary data]
```
...
## Development Mode (current runtime)

If validated datasets already exist locally and genuinely cover the requested AOI, the backend reuses them. The current validated cache covers approximately **lon 75.80–76.81, lat 17.89–18.61** (Latur–Osmanabad, Maharashtra) at 50 m resolution.

## Dynamic Mode (architecture)

When the cache does not cover a requested AOI, the acquisition architecture (providers in `backend/src/acquisition`) can discover, download, validate and preprocess authoritative source data. In the current synchronous API, an out-of-coverage request returns a truthful `DATA_COVERAGE_BLOCKER` (HTTP 422) rather than silently substituting geographically incorrect data; dynamic acquisition would need to be wired into the synchronous request path to extend coverage.
...
Authentication is intentionally **out of scope**. The platform runs in **Demo Mode** with a demo user and a development session. There are no accounts, login/signup, organizations, permissions, or persistent user identity. There is no government or institutional identity or security clearance.
...
Note: on some Windows machines Windows Application Control blocks the unsigned
native DLL in the rasterio 1.5.x wheel; requirements.txt therefore pins
`rasterio<1.5`. A working Python 3.12 environment was validated with
`rasterio==1.4.4`.
...
| Data acquisition architecture (providers) | Implemented (not wired into synchronous API) |
| Corridor Planner | Implemented |
| Site Finder | Implemented |
| Deterministic MCDA | Implemented |
| Deterministic explanation engine | Implemented |
| FastAPI backend | Implemented |
| Frontend (React + MapLibre) | Implemented |
| Authentication | Out of scope (Demo Mode) |
